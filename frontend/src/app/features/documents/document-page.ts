      ...
      @if (isStatuses) {
        ...
        @if (isCreateRoute) {
          <form class="panel create-form" (ngSubmit)="createDocument()">
            ...
            <div class="form-actions"><a routerLink="/documents" class="secondary-action">Cancelar</a><button class="primary-action" type="submit" [disabled]="creating() || !createForm.name.trim() || !createForm.code.trim() || !createForm.documentTypeId">{{ creating() ? 'Guardando…' : (isUploadRoute ? 'Crear y subir' : 'Crear documento') }}</button></div>
          </form>
        }
...
export class DocumentPage {
  ...
  readonly pagedDocuments = () => this.filteredDocuments().slice(this.page() * this.pageSize(), (this.page() + 1) * this.pageSize());
  readonly totalPages = () => Math.max(1, Math.ceil(this.filteredDocuments().length / this.pageSize()));
  readonly statuses = [
  ...
  selectInitialFile(event: Event): void { this.initialFile = (event.target as HTMLInputElement).files?.[0] || null; }
  createDocument(): void {
  ...
  loadTypes(): void {
    ...
    this.api.documents('', undefined, 0, 100, this.scope).subscribe({
  ...
  setSearch(event: Event): void { this.search.set((event.target as HTMLInputElement).value); this.page.set(0); }
  setStatus(event: Event): void { this.statusFilter.set((event.target as HTMLSelectElement).value); this.page.set(0); }
  ...
  clearFilters(): void {
    ...
    this.sortBy.set('updated'); this.page.set(0);
  }
  setSort(event: Event): void { this.sortBy.set((event.target as HTMLSelectElement).value); this.page.set(0); }
  setPageSize(event: Event): void { this.pageSize.set(Number((event.target as HTMLSelectElement).value)); this.page.set(0); }
  ...
  uploadVersion(document: ApiDocument, event: Event): void { const input=event.target as HTMLInputElement;const file=input.files?.[0];if(!file)return;this.uploadDocument=document;this.uploadFile.set(file);this.uploadReason='Carga de archivo'; }
